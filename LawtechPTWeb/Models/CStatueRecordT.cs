...
namespace LawtechPTWeb.Models
{
    ...
    public class CStatueRecordT : SysBaseModel
    {
        ...
        public object CheckValueExist(string ColumnName, object Value, ref bool IsExist, bool IsCreateMode = true)
        {
            ...
            List<CStatueRecordT> list = new List<CStatueRecordT>();
            ...
            if (IsCreateMode)
            {
                retObj = orm.ReadListToObjs<CStatueRecordT>(ColumnName + "=@" + ColumnName, ref list, ParList);
            }
            else
            {
                retObj = orm.ReadListToObjs<CStatueRecordT>(ColumnName + "=@" + ColumnName + " and SRID<>" + this.SRID.ToString(), ref list, ParList);
            }
