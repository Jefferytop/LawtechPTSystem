        ...
        private void contextMenuStrip_Trademark_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            switch (e.ClickedItem.Name)
            {
                    ...
                    if (trademarkManagementTDataGridView.CurrentRow != null)
                    {
                        ...
                        if (iLocker == -1 || iLocker == iWorkerID)
                        {
                            ...
                            if (MessageBox.Show("是否確定刪除【" + strName + "】", "確認視窗", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                            {
                                ...
                                log.DelContent = string.Format("本所案號:{0}\r\n商標名稱:{1}\r\n申請人:{2}\r\n商標樣式:{3}\r\n貴方案號:{4}\r\n對方案號:{5}", TMdel.TrademarkNo, TMdel.TrademarkName, TMdel.ApplicantNames, TMdel.StyleName, TMdel.MainCustomerRefNo,TMdel.OutsourcingTrademarkNo);
                                log.DelTitle = string.Format("刪除商標基本資料【{0}】", TMdel.TrademarkNo);
                                log.Create();
                ...
                case "toolStripMenuItem_CopyTrademark":
                    if (trademarkManagementTDataGridView.SelectedRows.Count > 0)
                    {
                        CopyForm.Trademark tm = new LawtechPTSystemByFirm.CopyForm.Trademark();
                ...
                case "toolStripButton_EditTM":
                    if (trademarkManagementTDataGridView.CurrentRow != null)
                    {
                        ...
                        if (iLocker == -1 || iLocker == iWorkerID)
                        {
                            ...
                            EditForm.EditTrademark Edit = new LawtechPTSystemByFirm.EditForm.EditTrademark();
                ...
                case "toolStripMenuItem_Controversy":
                    if (trademarkManagementTDataGridView.CurrentRow != null)
                    {
                        CopyForm.CopyTrademarkControversy copyControversy = new LawtechPTSystemByFirm.CopyForm.CopyTrademarkControversy();
                        copyControversy.TrademarkControversyID = (int)trademarkManagementTDataGridView.CurrentRow.Cells["TrademarkID"].Value;
                        copyControversy.ShowDialog();
                    }
                    break;
        ...
        private void contextMenuStrip_Event1_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            switch (e.ClickedItem.Name)
            {
                    ...
                    if (TMNotifyEventTDataGridView.CurrentRow != null)
                    {
                        ...
                        if (iLocker == -1 || iLocker == iWorkerID)
                        {
                            ...
                            if (MessageBox.Show("是否確定刪除【" + strName + "】", "確認視窗", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                            {
                                ...
                                Public.CTrademarkNotifyEventToFee NotifyEventToFee = new LawtechPTSystemByFirm.Public.CTrademarkNotifyEventToFee();
                                NotifyEventToFee.Delete("TMNotifyEventID=" + iKey.ToString(),"");

                                Public.CTrademarkNotifyEventToPayment NotifyEventToPayment = new LawtechPTSystemByFirm.Public.CTrademarkNotifyEventToPayment();
                                ...
                                //log.DelContent_InsertColumn = str[0];
                                //log.DelContent_InsertSQL = str[1];
                                log.DelContent = string.Format("事件內容:{0}\r\n本所來函日:{1}\r\n官方期限:{2}\r\n本所期限:{3}\r\n完成日期:{4}", TMdel.NotifyEventContent, TMdel.NotifyComitDate.HasValue?TMdel.NotifyComitDate.Value.ToString("yyyy/MM/dd"):"",  TMdel.DueDate.HasValue?TMdel.DueDate.Value.ToString("yyyy/MM/dd"):"", TMdel.AttorneyDueDate.HasValue? TMdel.AttorneyDueDate.Value.ToString("yyyy/MM/dd"):"",TMdel.FinishDate.HasValue? TMdel.FinishDate.Value.ToString("yyyy/MM/dd"):"");
                                log.DelTitle = string.Format("刪除商標事件記錄資料【{0}】", TMdel.NotifyEventContent);
                                log.Create();
                ...
                case "toolStripMenuItem_CopyEvent"://複製案件記錄
                    if (TMNotifyEventTDataGridView.CurrentRow != null)
                    {
                        CopyForm.CopyTrademarkEvent CopyEvent = new LawtechPTSystemByFirm.CopyForm.CopyTrademarkEvent();
                ...
                case "toolStripMenuItem_Paste":
                    if (trademarkManagementTDataGridView.CurrentRow != null)
                    {
                        if (MultipleKeyByCopy != "")
                        {
                            ...
                            {
                               List< Public.CTrademarkNotifyEvent> copyMulti = new List<LawtechPTSystemByFirm.Public.CTrademarkNotifyEvent>();
                        ...
                        if (GridView_Fee.CurrentRow.Cells["IsClosing"].Value == DBNull.Value || (bool)GridView_Fee.CurrentRow.Cells["IsClosing"].Value != true)
                        {
                            ...
                            if (iLocker == -1 || iLocker == iWorkerID)
                            {
                                ...
                                if (MessageBox.Show("是否確定刪除", "確認視窗", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                                {
                                    ...
                                    //log.DelContent_InsertColumn = str[0];
                                    //log.DelContent_InsertSQL = str[1];
                                    log.DelContent = string.Format("費用內容:{0}\r\n請款日期:{1}\r\n收據日期:{2}\r\n請款合計:{3}\r\n幣別:{4}\r\n請款單號:{5}", DelFee.FeeSubject, DelFee.RDate, DelFee.ReceiptDate, DelFee.Totall, DelFee.TMoney, DelFee.PPP);
                                    log.DelTitle = string.Format("刪除商標請款記錄資料【{0}】", DelFee.FeeSubject);
                                    log.Create();
                    ...
                    if (dataGridView_Billing.CurrentRow != null)
                    {
                        ...
                        if (iLocker == -1 || iLocker == iWorkerID)
                        {
                            ...
                            if (MessageBox.Show("是否確定刪除", "確認視窗", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                            {
                                ...
                                //log.DelContent_InsertColumn = str[0];
                                //log.DelContent_InsertSQL = str[1];
                                log.DelContent = string.Format("費用內容:{0}\r\n收件日期:{1}\r\n付款期限:{2}\r\n付款日期:{3}\r\n請款單編號:{4}\r\n實付金額:{5}", DelBilling.FeeSubject, DelBilling.ReciveDate, DelBilling.PayDueDate, DelBilling.PaymentDate, DelBilling.BillingNo,DelBilling.ActuallyPay.HasValue? DelBilling.ActuallyPay.Value.ToString("#,##0.##"):"0");
                                log.DelTitle = string.Format("刪除商標付款記錄資料【{0}】", DelBilling.FeeSubject);
                                log.Create();
...
費用科目：{2} 
       ...
       __________________________
                ...
                case "toolStripButton_ES_Edit":
                    if (dataGridView_EstimateCost.CurrentRow != null)
                    {
                        ...
                        if (iLocker == -1 || iLocker == iWorkerID)
                        {
                            ...
                            EditForm.EditTrademarkEstimateCost edit = new LawtechPTSystemByFirm.EditForm.EditTrademarkEstimateCost();
        ...
        private void linkLabel_mome_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            switch (link.Name)
            {
                ...
                case "linkLabel_RegisterProductItems":
                    pop = new LawtechPTSystemByFirm.US.PopMemo(txt_RegisterProductItems, true, link.Text);
                    break;
                ...
                case "linkLabel_AttorneyFirm":
                    pop = new LawtechPTSystemByFirm.US.PopMemo(txt_AttorneyFirm, true, link.Text);
                    break;  
                default:
                    pop = new LawtechPTSystemByFirm.US.PopMemo(txt_Remarks, true, link.Text);
                    break;
               ...
               for (int iCol = 0; iCol < ary.Length; iCol++)//法定階段
               {
                   ...
                   for (int iNo = 0; iNo < notify.Length; iNo++)
                   {
                       if (notify[iNo] != "")//細部程序
                       {
                           ...
                           if (content.Length == 2)
                           {
                               Public.CTrademarkNotifyEvent add = new LawtechPTSystemByFirm.Public.CTrademarkNotifyEvent();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            //{
            //    string TMno = dt.Rows[iRow]["TM_NO"].ToString();
            //    Public.CTrademarkControversyManagement tm = new LawtechPTSystemByFirm.Public.CTrademarkControversyManagement("TrademarkNo= '" + TMno + "'");
            ...
            //                if (content.Length == 2)
            //                {
            //                    Public.CTrademarkControversyNotifyEvent add = new LawtechPTSystemByFirm.Public.CTrademarkControversyNotifyEvent("1=0");
