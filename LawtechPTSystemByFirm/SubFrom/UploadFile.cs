...
namespace LawtechPTSystemByFirm.SubFrom
{
    public partial class UploadFile : Form
    {
        ...
        public void UploadTControlBinding()
        {
            ...
            txt_BuildWorker.DataBindings.Add("Text", uploadTBindingSource, "BuildWorker", true, DataSourceUpdateMode.OnValidation, "");
            ...
            txt_BuildDate.DataBindings.Add("Text", uploadTBindingSource, "BuildDate", true, DataSourceUpdateMode.OnValidation, "", "yyyy/MM/dd HH:mm");
        ...
        private void cmsUploadSelection_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            switch (e.ClickedItem.Name)
            {
                ...
                case "addToolStripMenuItem":
                    ...
                    if (uploadDT != null)
                    {
                        upload.NewRow = uploadDT.NewRow();
                    }

                    if (upload.ShowDialog() == DialogResult.OK)
                    {
                        uploadDT.Rows.Add(upload.NewRow);
                    }
                ...
                case "deleteToolStripMenuItem":
                    //逐筆刪除
                    if (dgvUploadFile.SelectedRows.Count > 0)
                    {
                        if (MessageBox.Show(string.Format("確定要刪除所選{0}筆資料嗎？", dgvUploadFile.SelectedRows.Count),
                            e.ClickedItem.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            ...
                            foreach (DataGridViewRow dgvRow in dgvUploadFile.SelectedRows)
                            {

                                dll.DeleteFolder(dll.GeneralFolderRootUpload + "\\" + dgvRow.Cells["UploadKey"].Value.ToString(), true);
                                uploadDT.Rows.Find(dgvRow.Cells["UploadKey"].Value).Delete();
                            }
                        }
                    }
                    else
                           ...
                           e.ClickedItem.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                            {
                                ...
                                uploadDT.Rows.Find(dgvUploadFile.CurrentRow.Cells["UploadKey"].Value).Delete();
                            }
                        }
                ...
                case "toolStripButton_Edit":
                    if (dgvUploadFile.CurrentRow != null)
                    {
                        EditForm.EditUploadFile edit = new LawtechPTSystemByFirm.EditForm.EditUploadFile();
        ...
        private void linkLabel1_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            US.PopMemo pop;
            pop = new LawtechPTSystemByFirm.US.PopMemo(txt_Description, true, link.Text);
