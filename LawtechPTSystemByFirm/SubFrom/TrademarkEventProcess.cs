...
namespace LawtechPTSystemByFirm.SubFrom
{
    ...
    public partial class TrademarkEventProcess : Form
    {
        ...
        {
            Public.PublicForm Froms = new LawtechPTSystemByFirm.Public.PublicForm();
                ...
                case "DelToolStripMenuItem":
                    ...
                    if (dataGridView_ProcessKind.CurrentRow != null)
                    {
                        ...
                        if (MessageBox.Show("是否確定刪除【" + strName + "】", "確認視窗", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                        {
                            ...
                            //string[] str = ProcessKind.GetInsertString(iKey);
                            //log.TableName = str[2];
                            ...
                            log.DelTitle = string.Format("刪除商標標準流程設定檔資料【{0}】", ProcessKind.ProcessKind);
                            log.Create();
                ...
                case "ToolStripMenuItem_DelProcess":
                    ...
                    if (processStepETDataGridView.CurrentRow != null)
                    {
                        ...
                        if (MessageBox.Show("是否確定刪除【" + strName + "】", "確認視窗", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                        {
                            ...
                            log.DelTime = DateTime.Now;
                            log.DelWorkerKey = Properties.Settings.Default.WorkerKEY;
                            //string[] str = ProcessStep.GetInsertString(iKey);
                            //log.TableName = str[2];
                            //log.DelContent_InsertColumn = str[0];
                            //log.DelContent_InsertSQL = str[1];
                            log.DelContent = string.Format("處理階段:{0}", ProcessStep.Handle);
                            log.DelTitle = string.Format("刪除商標標準流程設定檔處理階段資料【{0}--{1}】", dataGridView_ProcessKind.CurrentRow.Cells["ProcessKindName"].Value.ToString(), ProcessStep.Handle);
                            log.Create();
