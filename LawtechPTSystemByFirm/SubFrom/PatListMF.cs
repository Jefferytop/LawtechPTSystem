...
namespace LawtechPTSystemByFirm.SubFrom
{
    ...
    public partial class PatListMF : Form
    {
        ...
        public void TabFileBinding()
        {
            ...
            txt_FileNo.DataBindings.Add("Text", vPatentListBindingSource, "PatentNo", true, DataSourceUpdateMode.OnValidation, "", "");
            //提案名稱
...
delete [dbo].[PatComitEventToFeeT] where PatComitEventID in(select PatComitEventID from PatComitEventT where PatentID={0});
delete [dbo].PatComitEventToPaymentT where PatComitEventID in(select PatComitEventID from PatComitEventT where PatentID={0});
...
delete PatComitEventT where  PatentID={0};", GridView_File.CurrentRow.Cells["PatentID"].Value.ToString());
                                    ...
                                    //string[] str = Patent.GetInsertString(property_PatentID);
                                    //log.TableName = str[2];
                                    //log.DelContent_InsertColumn = str[0];
                                    //log.DelContent_InsertSQL = str[1];
                                    log.DelContent = string.Format("申請案卷號:{0}\r\n申請人:{1}\r\n申請案名稱:{2}\r\n貴方案號:{3}\r\n對方案號:{4}", Patent.PatentNo, Patent.ApplicantNames, Patent.Title, Patent.MainCustomerRefNo, Patent.AttorneyRefNo);
                                    log.DelTitle = string.Format("刪除專利基本資料【{0}】", Patent.PatentNo);
                                    log.Create();
                ...
                case "toolStripMenuItem_Copy":
                    if (GridView_File.CurrentRow != null)
                    {
                        CopyForm.Patent copy = new LawtechPTSystemByFirm.CopyForm.Patent();
        ...
        private void contextMenuStrip_Event0_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            switch (e.ClickedItem.Name)
            {
                    ...
                    if (GridView_PatComit.CurrentRow != null)
                    {
                        ...
                        if (iLocker == -1 || iLocker == iWorkerID)
                        {
                            ...
                            if (MessageBox.Show("是否確定刪除【" + ssName + "】", "確認視窗", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                            {
                                ...
                                log.DelTime = DateTime.Now;
                                log.DelWorkerKey = Properties.Settings.Default.WorkerKEY;
                                //string[] str = PatentComit.GetInsertString(iKey);
                                //log.TableName = str[2];
                                //log.DelContent_InsertColumn = str[0];
                                //log.DelContent_InsertSQL = str[1];
                                log.DelContent = string.Format("事件內容:{0}\r\n事件發生日:{1}", PatentComit.EventContent, PatentComit.CreateDate.HasValue?PatentComit.CreateDate.Value.ToString("yyyy/MM/dd"):"");
                                log.DelTitle = string.Format("刪除專利案件記錄資料【{0}】", PatentComit.EventContent);
                                log.Create();
                ...
                case "toolStripMenuItem_CopyComit":
                    if (GridView_PatComit.CurrentRow != null)
                    {
                        CopyForm.CopyPatentComitEvent copy = new LawtechPTSystemByFirm.CopyForm.CopyPatentComitEvent();
                    ...
                    if (dataGridView_EstimateCost.Rows.Count > 0)
                    {
                        ...
                        if (iLocker == -1 || iLocker == iWorkerID)
                        {
                            ...
                            if (MessageBox.Show("是否確定刪除", "確認視窗", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                            {
                                ...
                                log.DelTime = DateTime.Now;
                                log.DelWorkerKey = Properties.Settings.Default.WorkerKEY;
                                //string[] str = DelEstimateCost.GetInsertString(iPTEstimateCostID);
                                //log.TableName = str[2];
                                //log.DelContent_InsertColumn = str[0];
                                //log.DelContent_InsertSQL = str[1];
                                log.DelContent = string.Format("費用內容:{0}\r\n預估成本:{1}\r\n預估利潤:{2}\r\n實際報價:{3}", DelEstimateCost.FeeSubject, DelEstimateCost.EstimateCost.HasValue ? DelEstimateCost.EstimateCost.Value.ToString("#,##0.##") : "0", DelEstimateCost.EstimateProfit.HasValue ? DelEstimateCost.EstimateProfit.Value.ToString("#,##0.##") : "0",DelEstimateCost.RealPrice.HasValue? DelEstimateCost.RealPrice.Value.ToString("#,##0.##"):"0");
                                log.DelTitle = string.Format("刪除專利預估費用記錄資料【{0}】", DelEstimateCost.FeeSubject);
                                log.Create();
                ...
                case "toolStripButton_ES_Edit":
                    if (dataGridView_EstimateCost.CurrentRow != null)
                    {
                        ...
                        if (iLocker == -1 || iLocker == iWorkerID)
                        {
                            ...
                            EditForm.EditPatentEstimateCost edit = new LawtechPTSystemByFirm.EditForm.EditPatentEstimateCost();
        ...
        private void contextMenuStrip_Fee_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            switch (e.ClickedItem.Name)
            {
                ...
                case "toolStripMenuItem_BillingDel":  //刪除請款費用
                    ...
                    if (GridView_Fee.CurrentRow != null)
                    {
                        ...
                        if (MessageBox.Show("是否確定刪除", "確認視窗", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                        {
                            ...
                            //log.DelContent_InsertColumn = str[0];
                            //log.DelContent_InsertSQL = str[1];
                            log.DelContent = string.Format("費用內容:{0}\r\n請款日期:{1}\r\n請款單號:{2}\r\n請款合計:{3}\r\n幣別:{4}", DelFee.FeeSubject, DelFee.RDate.HasValue? DelFee.RDate.Value.ToString("yyyy/MM/dd"):"", DelFee.PPP,   DelFee.Totall.HasValue? DelFee.Totall.Value.ToString("#,##0.##"):"", DelFee.TMoney);
                            log.DelTitle = string.Format("刪除專利請款費用資料【{0}】", DelFee.FeeSubject);
                            log.Create();
                ...
                case "toolStripMenuItem_CopyFee":
                    if (GridView_Fee.CurrentRow != null)
                    {
                        CopyForm.CopyPatentFee copy = new LawtechPTSystemByFirm.CopyForm.CopyPatentFee();
                    ...
                    if (dataGridView_Billing.CurrentRow != null)
                    {
                        ...
                        if (iLocker == -1 || iLocker == iWorkerID)
                        {
                            ...
                            if (MessageBox.Show("是否確定刪除\r\n【" + sName + "】", "確認視窗", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                            {
                                ...
                                //log.DelContent_InsertColumn = str[0];
                                //log.DelContent_InsertSQL = str[1];
                                log.DelContent = string.Format("費用內容:{0}\r\n收件日期:{1}\r\n付款期限:{2}\r\n付款日期:{3}\r\n請款單編號:{4}\r\n金額合計:{5}\r\n幣別:{6}", DelBilling.FeeSubject,DelBilling.ReciveDate.HasValue? DelBilling.ReciveDate.Value.ToString("yyyy/MM/dd"):"",DelBilling.PayDueDate.HasValue? DelBilling.PayDueDate.Value.ToString("yyyy/MM/dd"):"",DelBilling.PaymentDate.HasValue? DelBilling.PaymentDate.Value.ToString("yyyy/MM/dd"):"", DelBilling.BillingNo,DelBilling.Totall.HasValue? DelBilling.Totall.Value.ToString("#,##0.##"):"", DelBilling.IMoney);
                                log.DelTitle = string.Format("刪除專利付款資料【{0}】", DelBilling.FeeSubject);
                                log.Create();
...
費用科目：{2} 
       ...
       __________________________
         金額小計： {3}  {7}", GridView_File.CurrentRow.Cells["PatentNo"].Value.ToString() ,
                        ...
                        payment.CostDept = GridView_File.CurrentRow.Cells["CountryName"].Value.ToString();
        ...
        private void linkLabel1_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            switch (link.Name)
            {
                ...
                case "linkLabel_Inventor":
                    pop = new LawtechPTSystemByFirm.US.PopMemo(txt_Inventor, true, link.Text);
                    break;
                ...
                case "linkLabel_ApplicantNames":
                    pop = new LawtechPTSystemByFirm.US.PopMemo(txt_Applicant, true, link.Text);
                    break;
                case "linkLabel_MainCustomer":
                    pop = new LawtechPTSystemByFirm.US.PopMemo(txt_MainCustomer, true, link.Text);
                    break;
                case "linkLabel_PatentAttorney":
                    pop = new LawtechPTSystemByFirm.US.PopMemo(txt_PatentAttorney, true, link.Text);
                    break;
                default:
