        ...
        private void contextMenuStrip1_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            switch (e.ClickedItem.Name)
            {
                case "AddPetition":
                    ...
                    AddPetition add = new AddPetition();
        ...
        private void PetitionSubjectT_ColumnChanged(object sender, DataColumnChangeEventArgs e)
        {
            if (e.Row.RowState == DataRowState.Unchanged)
            {
                ...
                if (IsChange)
                {
                    ...
                    {
                        Public.CSubject cs = new LawtechPTSystemByFirm.Public.CSubject("SID=" + e.ProposedValue.ToString());
        ...
        private void contextMenuStrip2_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            { 
                case "AddSubject":
                    ...
                    {
                        AddSubjectItem addItem = new AddSubjectItem();
                ...
                case "PasteSubjectToolStripMenuItem"://貼上大綱，包含對應的段落
                    if (dataGridView_Petition.CurrentRow != null)
                    {
                        ...
                        if (MessageBox.Show("是否貼上目前所選取的大綱及其對應的段落", "提示訊息", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                        {
                            ...
                            {
                                //大綱
                                Public.CPetitionSubject cps = new LawtechPTSystemByFirm.Public.CPetitionSubject("1=0");
                                ...
                                ////段落
                                Public.CParagraph cpp = new LawtechPTSystemByFirm.Public.CParagraph("PSID=" + dtSubjectTM.Rows[iSubject]["PSID"].ToString());
                                DataTable dtParagra = cpp.GetDataTable();
                                for (int iCP = 0; iCP < dtParagra.Rows.Count; iCP++)
                                {
                                    Public.CParagraph cpara = new LawtechPTSystemByFirm.Public.CParagraph("1=0");
                                ...
                                ////依據法條
                                //Public.CParagraphDetail cDetail = new LawtechPTSystemByFirm.Public.CParagraphDetail("ParaID=" + dtParaDetail.Rows[n]["ParaID"].ToString());
                                //for (int iDetail = 0; iDetail < cDetail.GetDataTable().Rows.Count; iDetail++)
                                //{
                                //    Public.CParagraphDetail add = new LawtechPTSystemByFirm.Public.CParagraphDetail("1=0");
            ...
            switch (e.ClickedItem.Name)
            {
                case "AddParag":
                    if (dataGridView_Subject.CurrentRow != null)
                    {
                        ...
                        AddPraagraph add = new AddPraagraph();
        ...
        private void dataGridView_Paragraph_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1&&e.ColumnIndex!=-1)
            {
                switch (dataGridView_Paragraph.Columns[e.ColumnIndex].Name)
                {
                    ...
                    case "ParagraphEN":
                        AddFrom.InputMemo input = new LawtechPTSystemByFirm.AddFrom.InputMemo((DataGridViewTextBoxCell)dataGridView_Paragraph.CurrentCell, true, dataGridView_Paragraph.Columns[e.ColumnIndex].Name);
                        break;
                }
            }
        }
