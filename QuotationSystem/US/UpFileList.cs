    ...
    public partial class UpFileList : Form
    {
        ...
        public string FileUploadPath
        {
            get {
                ...
                if (UploadMode == 1)
                {
                    strPath = Path.Combine( dll.GeneralBasedOnFolderRoot, mFileKey.ToString());
                }
                else if (UploadMode == 2)
                {
                    strPath = Path.Combine( dll.GeneralFolderRoot, mFileKey.ToString());
                }
                else if (UploadMode == 3)
                {
                    strPath = Path.Combine( dll.PatentFolderRoot, mFileKey.ToString());
                }
                else if (UploadMode == 4)
                {
                    strPath = Path.Combine( dll.TrademarkFolderRoot, mFileKey.ToString());
                }
                else if (UploadMode == 5)
                {
                    strPath = Path.Combine( dll.TrademarkControversyFolderRoot, mFileKey.ToString());
                }
                else if (UploadMode == 6)
                {
                    strPath = Path.Combine(dll.OfficePropertyFolderRoot, mFileKey.ToString());
                }
                else if (UploadMode == 10)
                {
                    strPath = Path.Combine( dll.KMFolderRoot, mFileKey.ToString());
                }
                else if (UploadMode == 40)
                {
                    strPath = Path.Combine(dll.CustomerFolderRoot, mFileKey.ToString());
                }
                else if (UploadMode == 50)
                {
                    strPath = Path.Combine( dll.FirmFolderRoot, mFileKey.ToString());
                }
                return strPath;
            }
        }
        ...
        private void UpFileList_Load(object sender, EventArgs e)
        {
            ...
            lab_FilePath.Text = FileUploadPath;
