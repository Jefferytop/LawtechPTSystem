...
namespace LawtechPTSystem.US
{
    ...
    public partial class SearchMain : UserControl
    {
        ...
        /// <summary>
        /// 記錄使用的表單 Patent,PATControlEvent
        /// </summary>        
        [Category("Custom"), DefaultValue("Patent"), Description("記錄使用的表單 Patent,PATControlEvent")]
        ...
        {
           SqlConnection conn = new SqlConnection(DBAccess.ConnectionListDefaultConnectionString);
           SqlDataAdapter sAdapter = new SqlDataAdapter("select * from PatConditionT where Type='" + strType + "' order by Sort", conn);
            DataTable dt = new DataTable(); 
            sAdapter.Fill(dt);
