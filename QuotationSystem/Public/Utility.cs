...
namespace LawtechPTSystem.Public
{
    public class Utility
    {
        ...
        public static int GetMaxSort(string TableName, string ColumnName= "Sort", string sqlQuery = "")
        {
            ...
            string strSQL = string.Format("Select max({0})+1 from {1} with(nolock) {2}", ColumnName, TableName, sqlQuery != "" ? "where " + sqlQuery : "");
        ...
        #region 檢查是否為合法的日期，否-清空資料;  是-保留keyin 資料 public static void CheckMaskedtextbox(MaskedTextBox mak)
        /// <summary>
        /// 檢查是否為合法的日期，否-清空資料;  是-保留keyin 資料
        /// </summary>
