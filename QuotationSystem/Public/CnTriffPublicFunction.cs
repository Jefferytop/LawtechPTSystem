...
namespace LawtechPTSystem.Public
{
    ...
    public class CnTriffPublicFunction
    {
        ...
        public static void GetClientFeeList(string strApplicantKey, string strApplicantMode, ref DataTable dtSource)
        {
            ...
            {
                DataColumn[] pk = { dtSource.Columns["ApplicantKey"], dtSource.Columns["Country"], dtSource.Columns["Kind"], dtSource.Columns["LargeEntity"] };
        ...
        /// <param name="strKindSN"></param>
        /// <param name="bLargeEntity"></param>
        /// <param name="IsPrint"></param>
        public static void UpdateClientFeeDetailList(string strApplicantKey, string strCountrySymbol, string strKindSN, string bLargeEntity,string strApplicantMode, string IsPrint)
        {

            string strSQL = string.Format(@"update ClientFeeT with(Rowlock) set  IsPrint='{5}'                                              
                                                Where   ApplicantKey = {0} and Country = '{1}' and 
                                                        Kind = '{2}' and LargeEntity = '{3}' and ApplicantMode={4}  ", strApplicantKey, strCountrySymbol, strKindSN, bLargeEntity, strApplicantMode, IsPrint);

            DBAccess dbhelper = new DBAccess();

            object obj = dbhelper.ExecuteNonQuery(strSQL);
