...
namespace LawtechPTSystem.Public
{
    ...
    class CommonFunctions
    {
        ...
        {
            string sSQL = string.Format("UPDATE {0} WITH(ROWLOCK) SET LockedWorker=null  where LockedWorker={1} ; UPDATE {0} WITH(ROWLOCK) set LockedWorker={1}  where {2}", TableName, iManagerKey, sWhere);
        ...
        {
            string sSQL = string.Format("UPDATE {0} WITH(ROWLOCK) SET LockedWorker=null  where {1}", TableName, sWhere);
        ...
        {
            string sSQL = string.Format("UPDATE {0} WITH(ROWLOCK) SET LockedWorker=null  where LockedWorker={1} ", TableName, iManagerKey.ToString());
