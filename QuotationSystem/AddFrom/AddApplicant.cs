...
namespace LawtechPTSystem.AddFrom
{
    public partial class AddApplicant : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            app.CheckValueExist("ApplicantSymbol", txt_ApplicantSymbol.Text, ref isWorkerSymbol);
            if (isWorkerSymbol)
            {
                MessageBox.Show("申請人代碼重覆，請重新再確認", "提示訊息");
                return;
            }
            ...
            {
                bool isExist = false;
                app.CheckValueExist("Account", txt_Account.Text, ref isExist);
                if (isExist)
                {
                    txt_Account.Focus();
                    MessageBox.Show("帳號重覆，請重新再確認", "提示訊息");
                    return;
                }
            }
