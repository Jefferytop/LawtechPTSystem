...
namespace LawtechPTSystem.SubFrom
{
    ...
    public partial class SetGridColumnT : Form
    {
        ...
        public void GetGridColumnT()
        {
            ...
            string strSQL = string.Format(@"SELECT *  from  GridColumnT with(nolock) where GridSymboID='{0}' and IsPublic<>'False'  order by Sort", comboBox_GridColumn.SelectedValue.ToString());
        ...
        {
            string strSQL = string.Format(@"SELECT Distinct GridSymboID from  GridColumnT with(nolock) ");
            ...
            if (e.Row.RowState == DataRowState.Unchanged)
            {
                ...
                switch (e.Column.ColumnName)
                {
                    ...
                    case "FontSize":
                        ...
                        {
                            float fsize =10;
                            bool isFontSize = float.TryParse(e.ProposedValue!= null? e.ProposedValue.ToString():"", out fsize);
                            if (isFontSize)
                            {
                                GC.FontSize = e.ProposedValue != DBNull.Value ? fsize.ToString() : "9";
                            }
                            else
                            {
                                e.ProposedValue = "";
        ...
        {
            bool isBool = DataGridView_GridColumn.Columns["GridColumnName"].Visible;
            DataGridView_GridColumn.Columns["GridColumnName"].Visible = !isBool;
            DataGridView_GridColumn.Columns["DataPropertyName"].Visible = !isBool;
        }
    }
}