...
namespace LawtechPTSystem.SubFrom
{
    ...
    public partial class TMNotifyDueMF : Form
    {
                ...
                case "DelToolStripMenuItem":
                    if (patNotifyDueTDataGridView.CurrentRow != null)
                    {
                        ...
                        if (MessageBox.Show("是否確定刪除【" + strName + "】", "確認視窗", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                        {
                            ...
                            Public.CTMNotifyDue.ReadOne(iKey, ref del_Due);
                            ...
                            log.DelTitle = string.Format("刪除「{0}」資料【{1}】", this.Text,del_Due.NotifyContent);
                            log.Create();
        ...
        private void patNotifyDueTDataGridView_SelectionChanged(object sender, EventArgs e)
        {
            if (patNotifyDueTDataGridView.CurrentRow != null)
            {
                if (patNotifyDueTDataGridView.CurrentRow.Cells["ProcessKEY"].Value != DBNull.Value && (int)patNotifyDueTDataGridView.CurrentRow.Cells["ProcessKEY"].Value != -1 && patNotifyDueTDataGridView.CurrentRow.Cells["ProcessKEY"].FormattedValue.ToString()!="")
                {
