...
using LawtechPTSystem.FTPClass;
using System.IO;
    ...
    public partial class UpFilePath : Form
    {
        ...
        public string FullPath
        {
            get {
                ...
                {  
                        return Path.Combine(txt_LocalhostPath.Text.Trim(), RootFolder);                    
                }
                else//區域網路路徑
                {
                    string sPath = Path.Combine(txt_IntranetPath.Text.Trim(), RootFolder);
                    return sPath;
                }
            }
        }
        ...
        private void but_OK_Click(object sender, EventArgs e)
        {
            try
            {
                if (FullPath != "")
                {
                    if (cb_IsFileServer.Checked) //啟用檔案上傳功能
                    {
                        if (radioButton1.Checked)//本機路徑
                        {
                            #region 本機路徑
                            //判斷是否有指定的資料夾
                            if (Directory.Exists(txt_LocalhostPath.Text))
                            {
                            ...
                            {
                                MessageBox.Show("找不到該路徑：" + FullPath);
                                return;
                            } 
                            #endregion
                        }
                            ...
                            #region 區域網路                            
                            //判斷是否有指定的資料夾
                            if (Directory.Exists(txt_IntranetPath.Text))
                            {
                                //判斷是否有OfficeSystemData資料夾
                                if (!Directory.Exists(lab_FullPath.Text))
                                {
                    ...
                    Properties.Settings.Default.FileServerLocalhostPath = txt_LocalhostPath.Text;

                    Properties.Settings.Default.IntranetPath = txt_IntranetPath.Text;
                    ...
                    {
                        MessageBox.Show("修改成功，已關閉檔案上傳功能 ", "提示");
                    }
        ...
        private void UpFilePath_Load(object sender, EventArgs e)
        {
            ...
            //FileServerType 0:本機路徑 ; 1:區域網路
            var itemFileServerType = StatueRecord.Find(x => x.StatusName == "FileServerType");
            if (itemFileServerType.Value == "0")
            {
