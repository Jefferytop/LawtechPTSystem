                ...
                if (e.Row.RowState == DataRowState.Unchanged)
                {
                    ...
                    switch (e.Column.ColumnName)
                    {
                        case "SearchAuthority":
                            wp.SearchAuthority = e.ProposedValue != DBNull.Value && (bool)e.ProposedValue;
                            break;
        ...
        private void contextMenuStrip1_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            switch (e.ClickedItem.Name)
            {
                ...
                case "AddToolStripMenuItem":
                    AddFrom.AddWorker add = new LawtechPTSystem.AddFrom.AddWorker();
                    add.ShowDialog();
                break;
        ...
        private void contextMenuStrip2_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            switch (e.ClickedItem.Name)
            {
                case "ToolStripMenuItem_Excel":
                    ...
                    break;
                case "toolStripMenuItem_CheckAuth":
                    int iProgramKind = (int)comboBox1.SelectedValue;
                    int iWorkerKey = (int)dataGridView1.CurrentRow.Cells["WorkerKey"].Value;

                    DataTable dtProgram = new DataTable();

                    Public.CProgram.ReadList(ref dtProgram, "ProgramKind=" + iProgramKind.ToString());

                    for (int i = 0; i < dtProgram.Rows.Count; i++)
                    {
                        int iProgramKEY = (int)dtProgram.Rows[i]["ProgramKEY"];
                        Public.CWorkerProgram wp = new Public.CWorkerProgram();
                        Public.CWorkerProgram.ReadOne(string.Format("ProgramKey ={0} and WorkerKey={1}", iProgramKEY.ToString(), iWorkerKey.ToString()), ref wp);

                        wp.AuthorizeCreate = true;
                        wp.AuthorizeModify = true;
                        wp.SearchAuthority = true;
                        wp.AuthorizeDelete = true;
                        wp.AuthorizeDownload = true;
                        wp.AuthorizeExport = true;
                        wp.AuthorizeUpload = true;
                        if (wp.PWID > 0)
                        {
                            wp.Update();
                        }
                    ...
                    MessageBox.Show("權限設定完成", "提示訊息");
                    break;
                case "toolStripMenuItem_CancelAuth":
                    int iProgramKindCancel = (int)comboBox1.SelectedValue;
                    int iiWorkerKey = (int)dataGridView1.CurrentRow.Cells["WorkerKey"].Value;

                    DataTable dtProgramCancel = new DataTable();

                    Public.CProgram.ReadList(ref dtProgramCancel, "ProgramKind=" + iProgramKindCancel.ToString());

                    for (int i = 0; i < dtProgramCancel.Rows.Count; i++)
                    {
                        int iProgramKEY = (int)dtProgramCancel.Rows[i]["ProgramKEY"];
                        Public.CWorkerProgram wp = new Public.CWorkerProgram();
                        Public.CWorkerProgram.ReadOne(string.Format("ProgramKey ={0} and WorkerKey={1}", iProgramKEY.ToString(), iiWorkerKey.ToString()), ref wp);

                        wp.AuthorizeCreate = false;
                        wp.AuthorizeModify = false;
                        wp.SearchAuthority = false;
                        wp.AuthorizeDelete = false;
                        wp.AuthorizeDownload = false;
                        wp.AuthorizeExport = false;
                        wp.AuthorizeUpload = false;
                        if (wp.PWID > 0)
                        {
                            wp.Update();
                        }
                        ...
                        {
                            wp.WorkerKEY = iiWorkerKey;
                            wp.ProgramKEY = iProgramKEY;
                            wp.Create();
                        }
                    }
                    comboBox1_SelectedIndexChanged(null, null);
                    MessageBox.Show("取消權限設定完成", "提示訊息");
                    break;
            }
        }
        #endregion
