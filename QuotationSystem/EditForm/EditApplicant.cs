...
namespace LawtechPTSystem.EditForm
{
    public partial class EditApplicant : Form
    {
        ...
        private void EditApplicant_Load(object sender, EventArgs e)
        {
            ...
            txt_CountryCitizenship.Text = app.CountryCitizenship;
            ...
            txt_PostalCode.Text = app.PostalCode;
            ...
            Check_T.Checked = app.T.HasValue ? app.T.Value : false;
            Check_C.Checked = app.C.HasValue ? app.C.Value : false;
            Check_L.Checked = app.L.HasValue ? app.L.Value : false;
            Check_E.Checked = app.E.HasValue ? app.E.Value : false;
            checkBox_LawVIP.Checked = app.LawVIP.HasValue ? app.LawVIP.Value : false;
        ...
        private void CountryCitizenshipTextBox()
        {
            ...
            txt_CountryCitizenship.AutoCompleteMode = AutoCompleteMode.Suggest;
            txt_CountryCitizenship.AutoCompleteSource = AutoCompleteSource.CustomSource;
            txt_CountryCitizenship.AutoCompleteCustomSource = acsc;
        }
        #endregion
        ...
        private void but_OK_Click(object sender, EventArgs e)
        {
            ...
            app.CheckValueExist("ApplicantSymbol", txt_ApplicantSymbol.Text.Trim(), ref isWorkerSymbol, false);
            if (isWorkerSymbol)
            {
                MessageBox.Show(string.Format("客戶代碼重覆 {0}，請重新再確認", txt_ApplicantSymbol.Text.Trim()), "提示訊息");
                return;
            }
            ...
            {
                bool isExist = false;
                app.CheckValueExist("Account", txt_Account.Text.Trim(), ref isExist,false);
                if (isExist)
                {
                    txt_Account.Focus();
                    MessageBox.Show( string.Format("網頁帳號重覆 {0}，請重新再確認", txt_Account.Text.Trim()), "提示訊息");
                    return;
                }
            } 
            #endregion

            app.ApplicantSymbol = txt_ApplicantSymbol.Text.Trim();
            app.ApplicantName = txt_ApplicantName.Text;
            app.ApplicantNameEn = txt_ApplicantNameEn.Text;
            app.CountryCitizenship = txt_CountryCitizenship.Text;
            app.Address = txt_Addr.Text;
            app.AddressEn = txt_AddrEn.Text;
            app.ContactAddr = txt_LiaisonAddr.Text;
            app.PostalCode = txt_PostalCode.Text.Trim();
            app.Account = txt_Account.Text.Trim();
            app.Password = txt_Password.Text.Trim();
        ...
        private void maskedTextBox_StartedDate_Leave(object sender, EventArgs e)
        {
