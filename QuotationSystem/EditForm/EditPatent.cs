...
namespace LawtechPTSystem.EditForm
{
    public partial class EditPatent : Form
    {
        ...
        private void EditPatent_Load(object sender, EventArgs e)
        {
            ...
            txt_GraceRemark.Text = pat.GraceRemark;
            if (pat.GracePeriod.HasValue)
            {
                maskedTextBox_GracePeriod.Text = pat.GracePeriod.Value.ToString("yyyy-MM-dd");
            }
            ...
            comboBox_DelegateType.SelectedValue = pat.DelegateType ==null ? 1 : pat.DelegateType;
            comboBox_DelegateType_SelectedIndexChanged(comboBox_DelegateType,null);

            if (pat.IsBySelf.HasValue)
            {
                //本所自行辦理
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            pat.GraceRemark = txt_GraceRemark.Text;
           ...
           if (IsCloseDate) pat.CloseDate = CloseDate;
            ...
            DateTime GracePeriod;
            bool IsGracePeriod = DateTime.TryParse(maskedTextBox_GracePeriod.Text, out GracePeriod);
            if (IsGracePeriod) pat.GracePeriod = GracePeriod;
            else pat.GracePeriod = null;
...
WHERE          (ApplicantKey in ({0}) ) and Quit<>1 ", ApplicantKey);
        ...
        #region 日期控件驗證，當焦點移開時，檢查日期是否合法
        /// <summary>
        /// 
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void maskedTextBox_EarlyPriorityDate_Leave(object sender, EventArgs e)
        {
